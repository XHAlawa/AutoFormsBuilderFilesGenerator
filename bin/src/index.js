...
var startup = /** @class */ (function () {
    ...
    startup.main = function () {
        ...
        var paresedConfig = JSON.parse(fs.readFileSync(configFileName, 'utf-8'));
        if (paresedConfig.input === null || paresedConfig.input === '') {
            console.log("Url Not Found");
            process.exit(1);
        }
        paresedConfig.formsOutput = mainDir + path_1.default.resolve(paresedConfig.formsOutput).replace(/[a-zA-Z]\:/g, '');
        console.log("\nReading JSON from ".concat(paresedConfig.input));
        http.get(paresedConfig.input, null, function (res) {
            var chunk = '';
            ...
            res.on('end', function () {
                ...
                new templateGenerator_1.default(paresedConfig).buildForm(body);
            });
        });
        return 0;
    };
